...
def const_filename_img(pat, directory):
    fn = str(pat) + ".png"
    return os.path.join(directory, fn)

def const_filename_roi(pat, abn, directory):
    token_list = [str(pat), str(abn)]
...
def sample_patches(img, roi_mask, out_dir, img_id, abn, pos, patch_size=256,
    ...
    while sampled_abn < nb_abn:
        ...
        if overlap_patch_roi((x,y), patch_size, roi_mask, cutoff=pos_cutoff):
            ...
            patch_img = imageio.core.util.Array(patch.astype('int32'))
...
def sample_hard_negatives(img, roi_mask, out_dir, img_id, abn,  
    ...
    while sampled_bkg < start_sample_nb + nb_bkg:
        ...
        if not overlap_patch_roi((x,y), patch_size, roi_mask, cutoff=neg_cutoff):
            ...
            patch_img = imageio.core.util.Array(patch.astype('int32'), high=patch.max(), low=patch.min(), mode='I')
            filename = basename + "_%04d" % (sampled_bkg) + ".png"
            fullname = os.path.join(bkg_out, filename)
            imageio.imwrite(fullname, patch_img)
            sampled_bkg += 1
...
def run(roi_mask_path_file, roi_mask_dir, pat_train_list_file, full_img_dir, 
        ...
        bkg_dir='background', pos_dir='mal', neg_dir='ben', verbose=True):
    ...
    roi_mask_path_df = pd.read_csv(roi_mask_path_file, header=0, sep=";")
    roi_mask_path_df = roi_mask_path_df.set_index(['File Name'])
    roi_mask_path_df.sort_index(inplace=True)
    # Read train set patient IDs and subset the table.
    pat_train_abn = pd.read_csv(pat_train_list_file, header=0)
    
    pat_train = pd.DataFrame(data={"File Name" : pat_train_abn["File Name"].unique()})
    ...
    pat_train_abn = pat_train_abn.values
        
    # Determine the labels for patients.
    pat_labs = []
    for pat, abn in pat_train_abn:
        # print(str(pat) + " " + str(abn))
        pathology = path_df.loc[pat]['Bi-Rads']
        malignant = 0
        if str(pathology) != str(2) and str(pathology) != str(3):
            malignant = 1
    ...
    def write_pat_list(fn, pat_list):
        with open(fn, 'w') as f:
            for pat, abn in pat_list:
                f.write(str(pat) + "," + str(abn) + "\n")
            f.close()
            
            
    if val_size > 0:
        pat_train_abn, pat_val_abn, labs_train, labs_val = train_test_split(
            pat_train_abn, pat_labs, stratify=pat_labs, test_size=val_size, random_state=12345)
        
        # pat_train = []
        # for pat, abn in pat_train_abn:
        #     pat_train.append(pat)
            
        # np.unique(pat_train)
            
        # pat_val = []
        # for pat, abn in pat_val_abn:
        #     pat_val.append(pat)
            
        # np.unique(pat_val)
        
        # if len(pat_val) > 1:
        #     val_df = roi_mask_path_df.loc[pat_val]
        # else:
        #     locs = roi_mask_path_df.index.get_loc(pat_val[0])
        #     val_df = roi_mask_path_df.iloc[locs]
        write_pat_list(os.path.join(val_out_dir, 'pat_lst.txt'), pat_val_abn.tolist())
        
    # if len(pat_train) > 1:
    #     train_df = roi_mask_path_df.loc[pat_train]
    # else:
    #     locs = roi_mask_path_df.index.get_loc(pat_train[0])
    #     train_df = roi_mask_path_df.iloc[locs]
    write_pat_list(os.path.join(train_out_dir, 'pat_lst.txt'), pat_train_abn.tolist())
    ...
    def do_sampling(pat_list_abn, out_dir):
        
        pat_list = []
        for pat, abn in pat_list_abn:
            pat_list.append(pat)
            
        np.unique(pat_list)
            
        for pat in pat_list:
            full_fn = const_filename_img(pat, full_img_dir)
            pathology = path_df.loc[pat]['Bi-Rads']
            try:
                ...
                # Read mask image(s).
                df = pd.DataFrame(pat_list_abn, columns=['File Name', 'abn_num'])
                abn_path = df.loc[df['File Name'] == pat]
                
                print(abn_path.values)
                print("\n\n\n\n")

                bkg_sampled = False
                
                for pat, abn in abn_path.values:
                    print(pat)
                    print(abn)
                    mask_fn = const_filename_roi(pat, abn, roi_mask_dir)
                    if target_width is None:
                        mask_img = read_resize_img(mask_fn, target_height=target_height, gs_255=True)
                    else:
                        mask_img = read_resize_img(mask_fn, target_size=(target_height, target_width), gs_255=True)
                    ...
                    if nb_hns_ > 0:
                        ...
                        hns_sampled = 0
                    pos = 1 if str(pathology) != str(2) and str(pathology) != str(3) else 0
    ...
    sys.stdout.flush()
    do_sampling(pat_train_abn, train_out_dir)
    print ("Done.")
    #####
    if val_size > 0.:
        print ("Sampling for val set")
        sys.stdout.flush()
        do_sampling(pat_val_abn, val_out_dir)
        print ("Done.")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--pos-dir", dest="pos_dir", type=str, default="calc_mal")
    parser.add_argument("--neg-dir", dest="neg_dir", type=str, default="calc_ben")
